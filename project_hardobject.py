...
True_f=lambda x,y:x*y*(1-x)*(1-y)*np.exp(-(x-0.5)**2-(y-0.5)**2)
#True_f=lambda x,y:x*y*(1-x)*(1-y)
nabla_f=lambda  x,y:-1*np.exp(-(x-0.5)**2-(y-0.5)**2)*((-2*(-x**2+x+1)+(1-2*x)**2*(-x**2+x+2))*(1-y)*y+(-2*(-y**2+y+1)+(1-2*y)**2*(-y**2+y+2))*x*(1-x))
...
for n in N:
    ...
    for element in tri_elements:
        ...
        #construct F
        Fe=pf.local_F([p1,p2,p3],nabla_f)
        for l3 in range(3):
    ...
    error_L2.append(np.linalg.norm(abs(u-Tu))/n)
...
plt.figure()
plt.scatter(np.log(np.array(N)),np.log(error_L2))
plt.show()
